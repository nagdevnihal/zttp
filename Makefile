...
release:
	...
	@if [ -f .env ]; then \
		...
		if [ -n "$$IP" ]; then \
			echo "Updating installers with SERVER_URL=http://$$IP:8555"; \
			sed -i "s|SERVER_URL=\".*\"|SERVER_URL=\"http://$$IP:8555\"|g" dist/install.sh; \
			sed -i "s|\$$ServerUrl = \".*\"|\$$ServerUrl = \"http://$$IP:8555\"|g" dist/install.ps1; \
		fi; \
	else \
